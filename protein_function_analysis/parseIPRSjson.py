...
for result in iprscan['results']:
  ...
  for match in result['matches']:
    ...
    ## iprscan = {'results' : [ {'matches': [{'entry': {'accession' : 'IPR001356' , 'goXRefs': 'name', 'id': 'GO' }}} ] }
    if 'entry' in match:
      entry=match['entry'] 
      if 'accession' in entry:
        ...
        if accession.startswith('IPR'):
          name=entry['name']
          print(f"IPR: {accession} {name}")
			
          for entry in match['entry']: 
            for goXRef in entry['goXRefs']:
              go_id = goXRef['id'] 
              go_name =  goXRef['name'] 
              goinfo=f"{go_id} {go_name}"
              print(f"\t{goinfo}")
							#count up each time we see each GO term it across all of our sequences in our gene set
              if goinfo not in interpro2go:
                interpro2go[goinfo]=0
              interpro2go[goinfo]+=1
